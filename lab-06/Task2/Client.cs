    ...
    public class Client{
        ...
        public void Start(){
            receivingTask = Task.Run(() =>{
                ...
                while (!finished){
                    try{
                        byte[] messageSizeBytes = new byte[1024];
                        int bytesRec = clientSocket?.Receive(messageSizeBytes) ?? 0;
                        if (bytesRec > 0){
                            int size;

                            bool success = int.TryParse(Encoding.UTF8.GetString(messageSizeBytes, 0, 4), out size);
                            if(success){
                                bool properMessageReceived = false;
                                while(!properMessageReceived){
                                    byte[] bytes = new byte[size];
                                    int bytesRec2 = clientSocket?.Receive(bytes) ?? 0;
                                    if(bytesRec2 > 0){
                                        properMessageReceived = true;                                        
                                        string data = Encoding.UTF8.GetString(bytes, 0, bytesRec2);
